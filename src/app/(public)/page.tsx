...
// Re-using preview components for consistency
...
const HomePageContent = ({ businessId }: { businessId: string }) => {
  ...
  if (error || !data) {
     return (
        <div className="flex h-screen w-full flex-col items-center justify-center bg-background text-center px-4">
            ...
            <h1 className="text-2xl font-bold">Página no Encontrada o sin Configurar</h1>
            <p className="text-muted-foreground mt-2 max-w-md">
            La página de inicio no ha sido configurada por el administrador. Por favor, selecciona un negocio principal en el panel de Superadmin {'>'} Configuración.
            </p>
        </div>
    );
  }
...
export default function WelcomePage() {
  ...
  const configDocRef = useMemoFirebase(() => 
    !firestore ? null : doc(firestore, 'globalConfig', 'system'),
    [firestore]
  );
  
  const { data: globalConfig, isLoading: isConfigLoading } = useDoc<GlobalConfig>(configDocRef);
  ...
  if (globalConfig && globalConfig.mainBusinessId) {
    return <HomePageContent businessId={globalConfig.mainBusinessId} />;
  }

  // Fallback to the original static welcome page if no main business is set
  return (
    <div className="relative h-[calc(100vh-8rem)]">
        <Image
          src="https://images.unsplash.com/photo-1588656909074-a9ff6d608eb9?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3NDE5ODJ8MHwxfHNlYXJjaHw4fHxuYXR1cmUlMjB3ZWxsbmVzc3xlbnwwfHx8fDE3NjIyMjAxMzN8MA&ixlib=rb-4.1.0&q=80&w=1080"
          alt="A serene nature scene representing wellness and health."
          fill
          className="object-cover"
      ...
      <div className="relative z-10 flex flex-col items-center justify-center h-full text-center text-white p-4">
        ...
        <p className="mt-4 max-w-2xl text-lg md:text-xl drop-shadow-md">
          Tu plataforma integral para la gestión de productos de salud y bienestar. Aún no se ha configurado una página de inicio.
        </p>
      </div>
    </div>
  );
